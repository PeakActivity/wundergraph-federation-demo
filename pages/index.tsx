...
import { useQuery, useUser, withWunderGraph } from '../components/generated/nextjs';
...
const Home: NextPage = () => {
	...
	c0-30.5,5.8-59.7,16.7-86.5L484.4,669l69-116.7l68.5,116.5l83.8-142.5H785C772,642.8,673.3,733.2,553.4,733.2z"
				...
				<p className="mt-6 text-lg text-slate-600 text-center max-w-3xl mx-auto dark:text-slate-400">
					...
					to combine multiple GraphQL APIs into a single GraphQL API through the Apollo Federation specification.
				</p>
			</div>
			<div className="relative flex flex-col items-center overflow-hidden p-8 sm:p-12">
				<div className="w-full max-w-xl rounded-2xl bg-blue-50 px-20 py-14">
					<div className="flex justify-center">
						<button
							onClick={() => user ? logout({ logoutOpenidConnectProvider: true }) : login('google')}
							className="bg-slate-900 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50 text-white font-semibold h-12 px-6 rounded-lg w-full flex items-center justify-center sm:w-auto dark:bg-sky-500 dark:highlight-white/20 dark:hover:bg-sky-400"
						>
							{user ? `Hey ${user.name}! Logout?` : 'Login'}
						</button>
					</div>
					<div className="mx-auto flex max-w-sm flex-col items-center">
						<p className="mt-3 mb-8 text-center text-black/80">
							This is the result of your{' '}
							<code className="font-mono font-medium text-amber-500 font-bold">TopProducts</code> operation. This
							operation calls internally the Product, Accounts Inventory and Review service.
						</p>
						<code className="p-3">{products.error ? "Login to view..." : JSON.stringify(products, null, 2)}</code>
					</div>
